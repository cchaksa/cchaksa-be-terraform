...
## 2026-05-12 prod scraper callback URL 원복

- Lambda 전환 테스트가 끝나 `SCRAPE_CALLBACK_BASE_URL`을 `https://api.cchaksa.com`으로 되돌렸다.
- live AWS에는 task definition `prod-scraper-worker:4`를 복제해 callback URL만 원복한 `prod-scraper-worker:5`를 등록했다.
- Pipe `prod-scraper-jobs-to-ecs` target task definition을 `prod-scraper-worker:5`로 갱신했다.
- 검증 결과:
  - Pipe 상태: `RUNNING`
  - Pipe target task definition: `arn:aws:ecs:ap-northeast-2:984762359128:task-definition/prod-scraper-worker:5`
  - worker image: `984762359128.dkr.ecr.ap-northeast-2.amazonaws.com/prod-scraper-worker:5b1a5bcc55ceeef980b019c12fb41d2f15609928`
  - `SCRAPE_CALLBACK_BASE_URL=https://api.cchaksa.com`
- 로컬 `tfvars/prod.tfvars`도 동일하게 `https://api.cchaksa.com`으로 원복했다.
