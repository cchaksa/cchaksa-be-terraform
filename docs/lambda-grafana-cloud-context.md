...
- 2026-03-28 Asia/Seoul / local
  ...
  - 전환 중 사용한 임시 Grafana Cloud bootstrap access policy/token과 stack service account/token은 작업 종료 후 삭제했다.
...
- 2026-03-28 Asia/Seoul / local
  ...
  - Grafana HTTP API로 대시보드 JSON 재조회: `jvm-micrometer-4701`, `spring-boot-19004` version `3` 반영 확인
...
- 현재 스크래핑 워커는 ECS Service가 아니라 RunTask 구조라 `AWS ECS Cluster` 대시보드의 service 기반 패널은 비어 있을 수 있다.
